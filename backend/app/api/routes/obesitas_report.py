"""Registri Obesitas — per-NIK registry routes.

Sibling of ``lipid_report.py`` / ``dm_report.py`` and the Registri Hipertensi
block in ``hipertensi_report.py``, over the same scraped EPUS/ASIK blobs but
surfacing the anthropometry (BB / TB / derived IMT). There is no separate scrape
job: this is a new view + Redis cache over data the existing scrape already
populates.
...
copy of the same sanitizer, as its three siblings do.

See ``app.services.obesitas_registry_scan`` for the scan, thresholds and syarat —
including why IMT is derived rather than read, and why the control window is 3–6
months rather than the DM registry's every-month rule.
"""
...
from app.schemas.obesitas_registry import (
    ObesitasRegistryDashboardOut,
    ObesitasRegistryRow,
    ObesitasRegistrySummaryOut,
)
from app.services.obesitas_registry_export import build_obesitas_registry_workbook
...
router = APIRouter(tags=["obesitas-report"])

# Same caching contract as the Hipertensi/DM/Dislipidemia registries (30h TTL >
# 24h nightly warm).
...
_WARM_REPORT_TYPE = "obesitas_registry"


def _registry_cache_key(puskesmas_id: uuid.UUID, year: int) -> str:
    return f"obesitas_registry:dashboard:{puskesmas_id}:{year}"


def warm_obesitas_registry(
    ...
    payload = scan_obesitas_registry(
...
def _load_registry_payload_or_warm(
    ...
    if payload is None:
        payload = warm_obesitas_registry(
...
def _registry_row(p: dict, pk_name: str, year: int) -> ObesitasRegistryRow:
    return ObesitasRegistryRow(puskesmas_name=pk_name, tahun_pelaporan=year, **p)


@router.get("/obesitas-reports/registry", response_model=ObesitasRegistryDashboardOut)
def obesitas_registry_dashboard(
    ...
    """Per-NIK Obesitas registry. One patient per row; the full baseline +
    ...
    if payload is None:
        ...
        return ObesitasRegistryDashboardOut(
            items=[],
            total=0,
            page=params.page,
            size=params.size,
    ...
    total = len(patients)
    # Per-band breakdown of the (search-filtered) rows for the header.
...
@router.get(
    "/obesitas-reports/registry/summary", response_model=ObesitasRegistrySummaryOut
)
def obesitas_registry_summary(
    puskesmas_id: uuid.UUID = Query(...),
    year: int = Query(..., ge=2000, le=2100),
    db: Session = Depends(get_db),
    principal: Principal = Depends(get_dashboard_principal),
) -> ObesitasRegistrySummaryOut:
    """Aggregate counts over the Obesitas registry (one puskesmas, year): total
    pasien (CKG), the two IMT bands, dan pasien dengan riwayat HT / DM (Ya).
    ...
    if payload is None:
        _request_registry_warm(rc, puskesmas_id, year)
        return ObesitasRegistrySummaryOut(
            total=0,
            riwayat_ht_ya=0,
            riwayat_dm_ya=0,
            cache_hit=False,
    ...
    return ObesitasRegistrySummaryOut(
        total=len(patients),
        **count_interpretasi_bands(patients),
...
def _export_filename(pk_name: str, year: int) -> str:
    ...
    gdp/hipertensi/dm/lipid report modules, which each carry their own copy."""
    safe = "".join(c if (c.isalnum() or c in " -_") else "_" for c in pk_name).strip()
    return f"Registri Obesitas - {safe or 'Puskesmas'} - {year}.xlsx"


@router.get("/obesitas-reports/registry/export")
def export_obesitas_registry(
    puskesmas_id: uuid.UUID = Query(...),
    year: int = Query(..., ge=2000, le=2100),
    db: Session = Depends(get_db),
    principal: Principal = Depends(get_dashboard_principal),
) -> StreamingResponse:
    """Download the Registri Obesitas as an .xlsx for (puskesmas, year). Emits
    ALL patients (no pagination/search). Unlike the list endpoint this warms
    SYNCHRONOUSLY on a cold cache — a download cannot return a ``computing``
    placeholder. See ``obesitas_registry_export``."""
    _authorize(principal, puskesmas_id)
    pk_name = db.scalar(select(Puskesmas.name).where(Puskesmas.id == puskesmas_id))
    if pk_name is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Puskesmas not found")
...
@router.delete(
    "/obesitas-reports/registry/cache", status_code=status.HTTP_204_NO_CONTENT
)
def clear_obesitas_registry_cache(
    puskesmas_id: uuid.UUID = Query(...),
    year: int = Query(..., ge=2000, le=2100),
    principal: Principal = Depends(get_dashboard_principal),
) -> None:
