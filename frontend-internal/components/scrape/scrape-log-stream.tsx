...
import { useScrapeStream, type StreamStatus } from "@/lib/hooks/use-scrape-stream";
import { useScrapeJobLog } from "@/lib/hooks/use-scrape";
...
export function ScrapeLogStream({
  jobId,
  enabled,
}: {
  jobId: string;
  ...
  const { lines, status } = useScrapeStream(jobId, enabled);
  return (
...
export function ScrapeLogHistory({ jobId }: { jobId: string }) {
  const q = useScrapeJobLog(jobId);
