...
import { DmRegistryTable } from "@shared/dm/components/dm-registry-table";
import { DmFormulaCard } from "@shared/dm/components/dm-formula-card";
import { useDmReportListStore } from "@shared/dm/dm-report-list-store";
import {
  useClearDmRegistryCache,
  useExportDmRegistry,
  useDmRegistry,
} from "@shared/dm/use-dm";
...
export function DmRegistryView({
  title,
  description,
  showClearCache,
  exportLabel,
  ...
  const puskesmasId = useDmReportListStore((s) => s.puskesmasId);
  const year = useDmReportListStore((s) => s.year);
  const q = useDmReportListStore((s) => s.q);
  const page = useDmReportListStore((s) => s.page);
  const setPuskesmasId = useDmReportListStore((s) => s.setPuskesmasId);
  const setYear = useDmReportListStore((s) => s.setYear);
  const setQ = useDmReportListStore((s) => s.setQ);
  const setPage = useDmReportListStore((s) => s.setPage);
  ...
  const registry = useDmRegistry(registryQuery, !!puskesmasId);
  const clearCache = useClearDmRegistryCache();
  const exportRegistry = useExportDmRegistry();
      ...
      <section className="space-y-4">
        ...
        {puskesmasId && registry.data && !computing && registry.data.computed_at && (
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[var(--muted-foreground)]">
            <span>
              ...
              pasien (CKG) · {registry.data.total_dm.toLocaleString()} diabetes
              melitus · {registry.data.total_prediabetes.toLocaleString()}{" "}
              prediabetes
            </span>
          ...
          <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed border-[var(--border)] py-16 text-sm text-[var(--muted-foreground)]">
            ...
            <div className="text-center">
              <div className="font-medium text-[var(--foreground)]">
                Loading registri diabetes melitus…
              </div>
        ...
        {puskesmasId && !loading && registry.data && (
          <>
            ...
            <DmRegistryTable items={registry.data.items} />
            <Pagination
